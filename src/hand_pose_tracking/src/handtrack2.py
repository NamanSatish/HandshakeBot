...
class HandDetector:
    ...
    def pubHandsPoint(self, frame, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            return frame, img, self.results.multi_hand_landmarks[0]
        return frame, img, None
...
if __name__ == '__main__':
    ...
    while capture.isOpened():
        ...
        frame, img, pt = hand_detector.pubHandsPoint(frame, draw=True)
        if pt is not None:
            pts[0].append(pt.landmark[0].x)
            pts[1].append(pt.landmark[0].y)
            pts[2].append(pt.landmark[0].z)
