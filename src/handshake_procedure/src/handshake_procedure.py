...
class HandshakeProcedure:
    def __init__(self, json_file):
        ...
        self.hand_coord_sub = rospy.Subscriber('hand_pose_base', PointStamped, self.hand_callback)
        self.robot_point_pub = rospy.Publisher('robot_point', PointStamped, queue_size=10)
    ...
    def get_ar_tag_origin(self):
        ...
        while True:
            response = input(f"Current Location is {self.current_hand_position.point.x},{self.current_hand_position.point.y},{self.current_hand_position.point.z}, if correct type y")
            if response == 'y':
        ...
        while rospy.Time.now() - start_time < rospy.Duration(step.duration):
            try:
                ...
                if step.axis_x:  # Mirror
                    target_point.point.x = 2 * self.ar_tag_origin.point.x - target_point.point.x
                else:  # Track
                    target_point.point.x = self.current_hand_position.point.x

                if step.axis_y:  # Mirror
                    target_point.point.y = 2 * self.ar_tag_origin.point.y - target_point.point.y
                else:  # Track
                    target_point.point.y = self.current_hand_position.point.y

                if step.axis_z:  # Mirror
                    target_point.point.z = 2 * self.ar_tag_origin.point.z - target_point.point.z
                else:  # Track
