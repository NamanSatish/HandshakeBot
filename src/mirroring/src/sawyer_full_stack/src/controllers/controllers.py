...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        print("alex says hi: ", error)
        self.integ_error = self.Kw * self.integ_error.astype(float) + error 
        controller_velocity = target_velocity + self.Kp @ error + self.Ki @ self.integ_error + self.Kd @ (target_velocity - current_velocity) 
