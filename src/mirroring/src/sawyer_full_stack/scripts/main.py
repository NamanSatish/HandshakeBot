...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="line",
        help="Options: line, circle.  Default: line",
    )
    parser.add_argument(
        "-controller_name",
        "-c",
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="How many waypoints for the :obj:`moveit_msgs.msg.RobotTrajectory`.  Default: 300",
    )
    parser.add_argument(
        "--log", action="store_true", help="plots controller performance"
    )
    ...
    robot_trajectory = get_trajectory(
        limb, kin, ik_solver, target_pos, target_orientation, args
    )
    ...
    if args.controller_name == "moveit":
        ...
        # execute the path using your own controller.
            ...
            robot_trajectory, rate=args.rate, timeout=args.timeout, log=args.log
        )
        if not done:
